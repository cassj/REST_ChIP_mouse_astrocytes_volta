...
while(my $this_row = <$input_fh>){
  ...
  #ditch any quotes.
  $this_row =~ s/\"//g;
  my ($chr, $start, $end, $identifier)=split("\t",$this_row);

  #all on the top strand 
  my $strand = 1;
 
  #aim for the middle (ish)
  my $pos = floor($start+($end-$start));
  ...
  if ($primer3_res->number_of_results){
    ...
    unless($good_primers){
      ...
      warn "No primers pass Tm thresholds";
      next;
      }
